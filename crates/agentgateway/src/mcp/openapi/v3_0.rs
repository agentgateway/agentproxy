...
use openapiv3::{
	...
	RequestBody as RequestBodyv3, Schema as Schemav3,
};
...
impl OpenAPISpecification for OpenAPI30Specification {
	fn parse_schema(&self) -> Result<Vec<(Tool, UpstreamOpenAPICall)>, ParseError> {
		let tool_defs: Result<Vec<_>, _> = self
			.spec
			.paths
			.iter()
			.map(
					...
					let items: Result<Vec<_>, _> = item
						...
						.map(
							|(method, op)| -> Result<(Tool, UpstreamOpenAPICall), ParseError> {
								...
								if let Some(body_ref) = &op.request_body {
									...
									if let Some(media_type) = body.content.get("application/json") {
										if let Some(schema_ref) = &media_type.schema {
											...
											let body_schema =
												serde_json::to_value(compatible_schema).map_err(ParseError::SerdeError)?;
								...
								// Handle parameters
								let mut param_schemas: HashMap<ParameterType, Vec<(String, Value, bool)>> =
									HashMap::new();
								...
								for param_ref in &op.parameters {
									...
									let param_type = match compatible_param.location {
										...
										ParameterLocation::Cookie => {
											return Err(ParseError::UnsupportedReference(
