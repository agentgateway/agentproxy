...
use openapiv3::{
	Parameter as Parameterv3, Schema as Schemav3, SchemaKind as SchemaKindv3, Type as Typev3,
};
...
impl ToCompatible<CompatibleSchema> for Schemav3 {
	fn to_compatible(&self) -> Result<CompatibleSchema, ParseError> {
		let mut compatible = CompatibleSchema {
			description: self.schema_data.description.clone(),
			default: self.schema_data.default.clone(),
			example: self.schema_data.example.clone(),
			nullable: self.schema_data.nullable,
			..Default::default()
		};
		...
		match &self.schema_kind {
			SchemaKindv3::Type(type_def) => {
				match type_def {
						...
						compatible.enum_values = if string_type.enumeration.is_empty() {
							...
							Some(
								string_type
									.enumeration
									.iter()
									.filter_map(|opt| opt.as_ref().map(|s| Value::String(s.clone())))
									.collect(),
							)
						};
					},
					...
					Typev3::Object(object_type) => {
						...
						for (prop_name, prop_schema_ref) in &object_type.properties {
							...
							if let openapiv3::ReferenceOr::Item(prop_schema) = prop_schema_ref {
								...
								compatible
									.properties
									.insert(prop_name.clone(), Box::new(prop_compatible));
							}
					...
					Typev3::Array(array_type) => {
						...
						// Handle items schema
						if let Some(openapiv3::ReferenceOr::Item(items_schema)) = &array_type.items {
			...
			SchemaKindv3::OneOf { one_of } => {
				...
				if let Some(openapiv3::ReferenceOr::Item(first_schema)) = one_of.first() {
			...
			SchemaKindv3::AnyOf { any_of } => {
				...
				if let Some(openapiv3::ReferenceOr::Item(first_schema)) = any_of.first() {
					return first_schema.to_compatible();
				}
				// If no schemas or all references, return a generic object schema
				compatible.schema_type = Some("object".to_string());
			...
			SchemaKindv3::Any(any_schema) => {
				...
				// Handle array items
				if let Some(openapiv3::ReferenceOr::Item(items_schema)) = &any_schema.items {
					let items_compatible = items_schema.to_compatible()?;
					compatible.items = Some(Box::new(items_compatible));
				}

				// Handle other any_schema fields that are available
				compatible.enum_values = compatible
					.enum_values
					.or_else(|| Some(any_schema.enumeration.clone()));
				...
				compatible.exclusive_minimum = compatible
					.exclusive_minimum
					.or(any_schema.exclusive_minimum);
				compatible.exclusive_maximum = compatible
					.exclusive_maximum
					.or(any_schema.exclusive_maximum);
