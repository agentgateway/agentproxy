		...
		let spec: OpenAPI = match version {
			...
			OpenAPIVersion::V3_1 => {
				let spec: openapiv3_1::OpenApi =
					yamlviajson::from_str(content_31).expect("Should parse 3.1");
...
"#;
		...
		let spec: OpenAPI = match version {
			...
			OpenAPIVersion::V3_1 => {
				let spec: openapiv3_1::OpenApi =
					yamlviajson::from_str(content_30).expect("Should parse 3.1");
				OpenAPI::V3_1(Arc::new(spec))
			},
		};
...
"#;
		...
		match parse_openapi_schema(&openapi_spec) {
			Ok(tools_and_calls) => {
				...
				let tool_names: Vec<&str> = tools_and_calls
					.iter()
...
"#;
				...
				if let Some(properties) = tool.input_schema.get("properties") {
					if let Some(props_obj) = properties.as_object() {
						...
						if !props_obj.is_empty() {
							...
							println!(
								"⚠ No request body properties processed yet (expected for current implementation)"
							);
						}
					}
				}
	...
	fn test_normalize_schema_v3_1_type_arrays() {
		...
		let result = openapi_31
			.normalize_schema_v3_1(&type_array_schema)
			.unwrap();
		...
		let result = openapi_31
			.normalize_schema_v3_1(&number_array_schema)
			.unwrap();
		...
		let result = openapi_31
			.normalize_schema_v3_1(&array_type_schema)
			.unwrap();
	...
	fn test_normalize_schema_composition_anyof() {
		...
		let result = openapi_31
			.normalize_schema_v3_1(&anyof_with_nullable)
			.unwrap();
		let anyof_array = result["anyOf"].as_array().unwrap();
	...
	fn test_process_parameter_v3_1_complex_types() {
		...
		let param: openapiv3_1::path::Parameter =
			serde_json::from_value(param_with_type_array).expect("Should parse parameter");
		...
		let param: openapiv3_1::path::Parameter =
			serde_json::from_value(param_with_composition).expect("Should parse parameter");
	...
	fn test_process_request_body_v3_1_nested_schemas() {
		...
		let request_body: openapiv3_1::request_body::RequestBody =
			serde_json::from_value(request_body_with_nested).expect("Should parse request body");
