...
impl OpenAPI31Specification {
	...
	pub fn process_parameter_v3_1(
		...
		if let Some(existing_desc) = param_schema.get("description") {
			param_schema["description"] = json!(format!(
				"{} (in: {})",
		...
		for (path, path_item) in &self.spec.paths.paths {
			...
			if let Some(operation) = &path_item.get {
				if let Some(operation_id) = &operation.operation_id {
					let tool = self.create_tool_from_operation(operation_id, "GET", path, operation)?;
					tools.push(tool);
				}
			}
			...
			if let Some(operation) = &path_item.post {
				if let Some(operation_id) = &operation.operation_id {
					let tool = self.create_tool_from_operation(operation_id, "POST", path, operation)?;
					tools.push(tool);
				}
			}
			...
			if let Some(operation) = &path_item.put {
				if let Some(operation_id) = &operation.operation_id {
					let tool = self.create_tool_from_operation(operation_id, "PUT", path, operation)?;
					tools.push(tool);
				}
			}
			...
			if let Some(operation) = &path_item.delete {
				if let Some(operation_id) = &operation.operation_id {
					let tool = self.create_tool_from_operation(operation_id, "DELETE", path, operation)?;
					tools.push(tool);
				}
			}
			...
			if let Some(operation) = &path_item.patch {
				if let Some(operation_id) = &operation.operation_id {
					let tool = self.create_tool_from_operation(operation_id, "PATCH", path, operation)?;
					tools.push(tool);
				}
			}
		}
